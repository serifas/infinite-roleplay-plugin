        ...
        if(ImGui.BeginChild("CREATE SYSTEM", new Vector2(550, 935)))
        {
            if(createSystem == true)
            {
                ...
                if (ImGui.Button("Create System"))
                {
                    ...
                    DataSender.CreateSystem(plugin.Configuration.username, statColors[0].GetHashCode().ToString(), SystemDescription, SystemImageBytes, int.Parse(maxStats), int.Parse(maxPointsPerStat), int.Parse(maxReductionCount), int.Parse(maxReductionPerStat), allocation_allowed, reduction_allowed, statCount);
                    DataSender.SendSystemStats(plugin.Configuration.username, statCount, statNames, statDescriptions, statColors);
                }
                ImGui.Separator();
